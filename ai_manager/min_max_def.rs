...
pub fn run(cells: Vec<State>, depth: i8, mut player: State) -> usize {
    ...
    for i in 0..size {
        if next_cells[i] == Empty {
            next_cells[i] = player;
            temp = run_recursive(next_cells.clone(), depth - 1, !maximize, next_player);
...
pub fn run_recursive(cells: Vec<State>, depth: i8, maximize: bool, final_player: State) -> i32 {
    let mut player = X;
    let mut next_player = O;
    if !maximize {
        player = O;
    ...
    if depth == 0 {
        return ai_manager::cost_function_1(cells.clone(), final_player);
    }
    ...
    return if maximize {
        ...
        for i in 0..size {
            if next_cells[i] == Empty {
                next_cells[i] = player;
                value = value.max(run_recursive(next_cells.clone(), depth - 1, !maximize, final_player));
                next_cells[i] = Empty;
            }
        }
        let (boolean, winner) = board::check_end(cells.clone());
        ...
        for i in 0..size {
            if next_cells[i] == Empty {
                next_cells[i] = player;
                value = value.min(run_recursive(next_cells.clone(), depth - 1, !maximize, final_player));
                next_cells[i] = Empty;
            }
        }
        let (boolean, winner) = board::check_end(cells.clone());
